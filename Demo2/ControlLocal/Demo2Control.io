...
void loop() {
  ...
  switch(state){  //changes motor position depending on input
    ...
    desiredRotPos = -1.5 + amountRotated;
    ...
    if((current_time >= timeState1 + 2) and (state2trig == 0)){
      state = 2;
      state2trig = 1;
      markerAngle = (((numberArray[1]/10.0)*(PI/180))*(1-.04458599)) ;
